...
function init(settings) {
    manager._options = _.extend(manager._options, settings.options);
...
function tasks() {
    _.forEach(manager.settings, function(value, key) {
        ...
        var settings = ( _.isObject(task.settings) && !_.isArray(task.settings) )
        ...
        if (_.isObject(task) && task.name) {
            ...
            _.forEach(subtasks, function(subtask){
                ...
                if (manager.settings[task.name][configKey] === false) {
                    ...
                    } else if (!manager.settings[task.name].callback && !manager.settings[task.name].dependencies.length) {
                        delete manager.settings[task.name];
                    }
                } else {
                    var settings;
...
function bundles() {
    ...
    var bundles = manager.settings.bundles;

    bundles.forEach(function(bundleName){
        var bundle = _loadTask(bundleName, {});
        manager.settings[bundleName] = bundle;
    });
}
