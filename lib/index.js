...
function subtasks(task) {
    ...
    _.forEach(subtasks, function(subtask, key) {
        var realName = key.replace(task.name+':','');
        var index = (subtask.config) ? subtask.config : realName;
        var config = task.settings[index];

        delete subtasks[key];

        if (config === undefined || config) {
            subtasks[realName] = subtask;
        } else {
            _removeItem(task.sequence, key);
            _removeItem(task.sequence, realName);
        }
